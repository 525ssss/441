...
// 修改登录按钮的事件处理函数，设置登录cookie
document.getElementById('login-btn').addEventListener('click', function () {
  const username = document.getElementById('username').value;
  const password = document.getElementById('password').value;
  if (username && password) {
    setCookie('username', username, 7); // 设置cookie，有效期为7天
    window.location.href = 'https://525ssss.github.io/441/PF/tk.html'; // 登录成功后跳转到目标页面
  } else {
    alert('Invalid username or password');
  }
});

// 确保在登录页面的表单提交后也进行登录状态检查
function submitForm(event) {
  event.preventDefault();
  if (validateForm()) {
    const username = document.getElementById('username').value;
    const password = document.getElementById('password').value;
    if (username && password) {
      setCookie('username', username, 7); // 设置cookie
      window.location.href = 'https://525ssss.github.io/441/PF/tk.html'; // 登录成功后跳转
    }
  }
}
